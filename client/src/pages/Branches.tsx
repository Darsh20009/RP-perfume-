...
export default function Branches() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-[#faf8f5] to-white" dir={isRTL ? "rtl" : "ltr"} data-testid="page-branches">
      {/* Hero */}
      <div className="relative bg-gradient-to-br from-[#1a2744] via-[#27325a] to-[#1a2744] text-white overflow-hidden">
        <div className="absolute inset-0 opacity-10" style={{ backgroundImage: "radial-gradient(circle at 20% 30%, #c9a96e 0%, transparent 50%), radial-gradient(circle at 80% 70%, #c9a96e 0%, transparent 50%)" }} />
        <div className="container relative px-4 py-20 md:py-28 text-center">
          <Building className="h-12 w-12 mx-auto mb-6 text-[#c9a96e]" />
          <h1 className="text-4xl md:text-6xl font-black tracking-tight mb-4">{title}</h1>
          <div className="w-24 h-px bg-[#c9a96e] mx-auto mb-6" />
      ...
      <div className="container px-4 py-12 md:py-16">
        {isLoading && (
          <div className="flex items-center justify-center py-24"><Loader2 className="h-10 w-10 animate-spin text-[#c9a96e]" /></div>
        )}
              ...
              <div key={b.id} data-testid={`card-branch-${b.id}`} className="group bg-white rounded-2xl overflow-hidden shadow-lg hover:shadow-2xl transition-all duration-500 border border-gray-100">
                ...
                <div className="relative h-56 bg-gradient-to-br from-[#f5f0eb] to-[#faf8f5] overflow-hidden">
                  {b.latitude && b.longitude ? (
                    ...
                    <div className="flex items-center justify-center h-full"><Building className="h-16 w-16 text-[#c9a96e]/30" /></div>
                  )}
                </div>

                <div className="p-6 md:p-8 space-y-5">
                  <div>
                    <h2 className="text-2xl md:text-3xl font-black text-[#1a2744] tracking-tight" data-testid={`text-branch-name-${b.id}`}>{displayName}</h2>
                    {b.city && <p className="text-xs uppercase tracking-widest text-[#c9a96e] font-black mt-1">{b.city}</p>}
                  </div>

                  <div className="space-y-3 text-sm">
                    {displayAddr && (
                      <div className="flex items-start gap-3 text-gray-700">
                        <span className="bg-[#c9a96e]/10 p-2 rounded-lg text-[#c9a96e] shrink-0"><MapPin className="h-4 w-4" /></span>
                        <span className="font-medium leading-relaxed pt-1.5">{displayAddr}</span>
                      </div>
                    )}
                    {b.phone && (
                      <a href={`tel:${b.phone}`} className="flex items-center gap-3 text-gray-700 hover:text-[#c9a96e] transition-colors group/item">
                        <span className="bg-[#c9a96e]/10 p-2 rounded-lg text-[#c9a96e] group-hover/item:bg-[#c9a96e] group-hover/item:text-white transition-colors shrink-0"><Phone className="h-4 w-4" /></span>
                        <span className="font-bold" dir="ltr">{b.phone}</span>
                      </a>
                    )}
                    {b.email && (
                      <a href={`mailto:${b.email}`} className="flex items-center gap-3 text-gray-700 hover:text-[#c9a96e] transition-colors group/item">
                        <span className="bg-[#c9a96e]/10 p-2 rounded-lg text-[#c9a96e] group-hover/item:bg-[#c9a96e] group-hover/item:text-white transition-colors shrink-0"><Mail className="h-4 w-4" /></span>
                        <span className="font-bold" dir="ltr">{b.email}</span>
                      </a>
                    )}
                    {b.hours && (
                      <div className="flex items-start gap-3 text-gray-700">
                        <span className="bg-[#c9a96e]/10 p-2 rounded-lg text-[#c9a96e] shrink-0"><Clock className="h-4 w-4" /></span>
                  ...
                  {b.isPickupEnabled !== false && (
                    <div className="bg-[#c9a96e]/5 border border-[#c9a96e]/20 px-4 py-2 rounded-lg">
                      <p className="text-xs font-black text-[#c9a96e] uppercase tracking-wider">
                  ...
                  <div className="grid grid-cols-2 gap-3 pt-2">
                    <a
                      ...
                      target="_blank"
                      rel="noreferrer"
                      className="flex items-center justify-center gap-2 px-4 py-3 bg-[#1a2744] text-white text-xs font-black uppercase tracking-widest rounded-lg hover:bg-[#27325a] transition-colors"
                    >
                    ...
                    <a
                      ...
                      target="_blank"
                      rel="noreferrer"
                      className="flex items-center justify-center gap-2 px-4 py-3 bg-white border-2 border-[#c9a96e] text-[#c9a96e] text-xs font-black uppercase tracking-widest rounded-lg hover:bg-[#c9a96e] hover:text-white transition-colors"
                    >
                      <Navigation className="h-3.5 w-3.5" />
